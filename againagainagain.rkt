...
; Collect odds and evens separately
; one collector to take sum of each
; one collector returns lists

(define (lpair o e)
  (build o e))

(define (sumpair o e)
  (build (addtup o) (addtup e)))
...
(define (oe l col)
  (cond
    ((null? l)
     (col '() '()))
    ((even? (car l))
     (oe (cdr l) (lambda (o e) (col o (cons (car l) e)))))
    (else
     (oe (cdr l) (lambda (o e) (col (cons (car l) o) e))))))

(t? '(oe '(2) lpair) '(() (2)))
(t? '(oe '(2 1) lpair) '((1) (2)))
(t? '(oe '(2 1 4) lpair) '((1) (2 4)))
(t? '(oe '(2 1 4 3) lpair) '((1 3) (2 4)))
(t? '(oe '(2 1 4 3) sumpair) '(4 6))
...
(define (keep-looking a n lat)
  (cond
    ; ((eq? n 1) #f) fix the partial...guess thats not what they wanted...
    ((number? n) (keep-looking a (pick n lat) lat))
    (else (eq? n a))))


(t? '(looking 'caviar '(6 2 4 caviar 5 7 3)) #t)
(t? '(looking 'caviar '(6 2 grits caviar 5 7 3)) #f)


(t? '(shift '((a b) c)) '(a (b c)))
(t? '(shift '((a b) (c d))) '(a (b (c d)))
